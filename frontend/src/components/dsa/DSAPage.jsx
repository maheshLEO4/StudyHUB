      ...
      {loading ? <div style={{textAlign:'center',paddingTop:40}}><Spinner size={28}/></div>
        ...
        : problems.map(p=>(
          <div key={p._id} className="dsa-item">
            <button className={`status-circle status-circle--${p.status}`} onClick={()=>cycleStatus(p)} title="Click to change status">
              {p.status==='completed' && <Icon name="check" size={13}/>}
              {p.status==='in-progress' && <div style={{width:8,height:8,borderRadius:'50%',background:'white'}}/>}
            </button>
            <div style={{flex:1,minWidth:0}}>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span className="badge badge-gray" style={{color:STATUS_COLOR[p.status]}}>{STATUS_LABEL[p.status]}</span>
              </div>
              {p.topic && <div className="text-xs text-muted">{p.topic}</div>}
            </div>
            <div className="flex gap-1 items-center">
              {p.url && <a href={p.url} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm btn-icon" title="Open problem"><Icon name="eye" size={13}/></a>}
              <button className="btn btn-ghost btn-sm btn-icon" onClick={()=>{setForm({title:p.title,url:p.url||'',topic:p.topic||'',difficulty:p.difficulty,status:p.status,notes:p.notes||''});setEditing(p);setShowModal(true);}}><Icon name="edit" size={13}/></button>
              <button className="btn btn-ghost btn-sm btn-icon" onClick={()=>deleteProblem(p._id)}><Icon name="trash" size={13}/></button>
            </div>
          </div>
        ))
      }
