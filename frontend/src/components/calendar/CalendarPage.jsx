...
const CalendarPage = () => {
  ...
  const dateStr = (d,other) => {
    const m = other&&d>15?month-1:other?month+1:month;
    const y = (other&&d>15&&month===0)?year-1:(other&&d<15&&month===11)?year+1:year;
    return `${y}-${String(Math.max(0,Math.min(11,m))).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
  };

  const todayStr = today();
  const selStr = selected ? `${year}-${String(month).padStart(2,'0')}-${String(selected).padStart(2,'0')}` : null;
      ...
      <div className="grid-2" style={{gap:24,alignItems:'start'}}>
        ...
        <div className="card" style={{padding:20}}>
          ...
          <div className="cal-grid">
            {cells.map((cell,i)=>{
              const ds = !cell.other ? `${year}-${String(month).padStart(2,'0')}-${String(cell.day).padStart(2,'0')}` : null;
